...
public class DynamicResourceController {
  private final JdbcTemplate jdbc; public DynamicResourceController(JdbcTemplate jdbc){this.jdbc=jdbc;}
  ...
  @GetMapping("/definitions") List<Definition> definitions(Authentication auth){var t=tenant(auth);return jdbc.query("select * from dynamic_resource.definition where tenant_id=? order by name",(r,n)->definition(r),t);}
  @PostMapping("/definitions") @ResponseStatus(HttpStatus.CREATED) @Transactional Definition createDefinition(@Valid @RequestBody DefinitionCreate request,Authentication auth){
    requireAdmin(auth); validateSchema(request.schema()); var id=UUID.randomUUID(); var t=tenant(auth); var actor=account(auth);
  ...
  @GetMapping("/{resourceKey}/records") PageResult records(@PathVariable String resourceKey,@RequestParam(defaultValue="0") int page,@RequestParam(defaultValue="25") int size,@RequestParam(defaultValue="") String q,Authentication auth){
    var t=tenant(auth); var d=definitionByKey(resourceKey,t); int safeSize=Math.max(1,Math.min(size,100)),safePage=Math.max(page,0); String search="%"+q.toLowerCase(Locale.ROOT)+"%";
    long total=jdbc.queryForObject("select count(*) from dynamic_resource.record where tenant_id=? and definition_id=? and status='ACTIVE' and (?='' or lower(data::text) like ?)",Long.class,t,d.id(),q,search);
    var items=jdbc.query("select r.*,? resource_key from dynamic_resource.record r where tenant_id=? and definition_id=? and status='ACTIVE' and (?='' or lower(data::text) like ?) order by updated_at desc,id limit ? offset ?",(r,n)->record(r),resourceKey,t,d.id(),q,search,safeSize,safePage*safeSize);
  ...
  @PostMapping("/{resourceKey}/records") @ResponseStatus(HttpStatus.CREATED) @Transactional RecordItem create(@PathVariable String resourceKey,@RequestBody JsonNode data,Authentication auth){
    var t=tenant(auth);var d=definitionByKey(resourceKey,t);validateData(d.schema(),data);var id=UUID.randomUUID();var actor=account(auth);
  ...
  @GetMapping("/{resourceKey}/records/{id}") RecordItem get(@PathVariable String resourceKey,@PathVariable UUID id,Authentication auth){return getRecord(id,resourceKey,tenant(auth));}
  @PutMapping("/{resourceKey}/records/{id}") @Transactional RecordItem update(@PathVariable String resourceKey,@PathVariable UUID id,@RequestBody JsonNode data,@RequestHeader("If-Match") int expectedVersion,Authentication auth){
    var t=tenant(auth);var d=definitionByKey(resourceKey,t);validateData(d.schema(),data);int next=expectedVersion+1;
  ...
  @DeleteMapping("/{resourceKey}/records/{id}") @ResponseStatus(HttpStatus.NO_CONTENT) @Transactional void archive(@PathVariable String resourceKey,@PathVariable UUID id,Authentication auth){var t=tenant(auth);var d=definitionByKey(resourceKey,t);int c=jdbc.update("update dynamic_resource.record set status='ARCHIVED',record_version=record_version+1,updated_at=now() where id=? and tenant_id=? and definition_id=? and status='ACTIVE'",id,t,d.id());if(c==0)throw new ApiProblem(HttpStatus.NOT_FOUND,"RECORD_NOT_FOUND","Record không tồn tại");audit(auth,"DYNAMIC_RECORD_ARCHIVED",resourceKey,id);}
  @GetMapping("/{resourceKey}/records/{id}/history") List<Revision> history(@PathVariable String resourceKey,@PathVariable UUID id,Authentication auth){var t=tenant(auth);definitionByKey(resourceKey,t);return jdbc.query("select * from dynamic_resource.revision where tenant_id=? and record_id=? order by record_version desc",(r,n)->new Revision(r.getObject("id",UUID.class),r.getInt("record_version"),r.getString("operation"),readJson(r.getString("data")),r.getObject("actor_id",UUID.class),r.getTimestamp("occurred_at").toInstant()),t,id);}
