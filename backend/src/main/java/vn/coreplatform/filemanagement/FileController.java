...
public class FileController {
  ...
  private void audit(Authentication a,String action,UUID id){jdbc.update("insert into audit.event(id,actor_id,actor_email,tenant_key,action,resource_type,resource_id,result,occurred_at) values(?,?,?,?,?,'FILE',?,'SUCCESS',now())",UUID.randomUUID(),permissions.account(a),a.getName(),permissions.tenant(a).toString(),action,id.toString());}
}
